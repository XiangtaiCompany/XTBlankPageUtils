...
+ (void)xt_showView:(UIView *)view hasData:(BOOL)hasData image:(UIImage *)image title:(NSString *)title yOffset:(CGFloat)yOffset {
    ...
    for (UIView *temp in view.subviews) {
        if ([temp isKindOfClass:[XTBlankPageUtils class]]) {
    ...
    XTBlankPageUtils *bgView = [[XTBlankPageUtils alloc] init];
...
+ (void)xt_networkConnectionErrorShowView:(UIView *)view image:(UIImage *)image title:(NSString *)title yOffset:(CGFloat)yOffset {
    
    [XTBlankPageUtils xt_showView:view hasData:NO image:image title:title yOffset:yOffset];
}
